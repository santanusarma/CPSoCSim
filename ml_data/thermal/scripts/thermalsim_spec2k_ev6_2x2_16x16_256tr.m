...
power_traces=[randi(NoOfBenchmarks,1), 3; 3, randi(NoOfBenchmarks,1)]
...
simtime=hotspot_grid_sim(ptrace,mpsoc_flp_file,mpsoc_config_file,ss_trace, nrows, ncols)
...
%cd ('../power_data/ptrace')
%steady state trace file
ss_trace_fl=[getfilename(powertr_file),'.grid', nrows, 'x', ncols, '.steady'];

eval(['!mv ', ss_trace_fl, '  ../power_data/ptrace/mpsoc_traces/'])

%eval(['!mv ', getfilename(powertr_file),'.grid', nrows, 'x', ncols, '.steady', '  ./mpsoc_traces/'])
cd ('../power_data/ptrace')
