function [ PowerTrace] = plot_power_trace( power_trace_file)
...
fid=fopen(power_trace_file, 'r')
fid_new=fopen('trace.txt', 'wb')
...
power_trace=dlmread('trace.txt')
[time_steps NoOfUnits]=size(power_trace)
...
%     figure(i)
%     plot(power_trace(:,i))
%     title(['Power trace for   ', BlockName{i}])
%     grid on;
%     pause (2)
%     close all
% end

PowerTrace={BlockName, power_trace};
