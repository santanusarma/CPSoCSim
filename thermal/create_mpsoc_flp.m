function [mpsoc_flp, mpsoc_flp_file]= create_mpsoc_flp(single_core_flp, nx, ny)
...
if isa(single_core_flp, 'char')
    ...
    mpsoc_flp_file=['mpsoc_', name, '_' num2str(nx), 'x', num2str(ny), '.flp'];
    
elseif isa (single_core_flp, 'struct')
    
    struct_str=sprintf('%s', inputname(1))
    
    mpsoc_flp_file=['mpsoc_', struct_str, '_' num2str(nx), 'x', num2str(ny), '.flp'];
else
    error('Unknown flp format')
end
...
%Print the mpsoc file
struct2flp(mpsoc_flp,mpsoc_flp_file, '\t');
