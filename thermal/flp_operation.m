function [FloorPlanCell FloorPlan]= flp_operation(hotspot_floor_plan,shift,Cord, scale, sf)
...
% INPUTS:
% hotspot_floor_plan = string with file name with extension/ flp struct
...
if isa(hotspot_floor_plan, 'struct')
    
    unit_name_1= hotspot_floor_plan.Labels;
    width_1=hotspot_floor_plan.w;
    height_1= hotspot_floor_plan.h;
    x_left_1= hotspot_floor_plan.x;
    y_bottom_1=hotspot_floor_plan.y;
    NoOfBlocks=hotspot_floor_plan.NoOfBlocks;
    EmptyLabels=hotspot_floor_plan.EmptyLabels;
    W=hotspot_floor_plan.Width;
    H=hotspot_floor_plan.Height;
...
%get the actual base of the provided floorplan 
BaseX_current= min(x_left_1);
BaseY_current= min(y_bottom_1);

%The base of the modified floorplan needed is [BaseX BaseY]
 
%Generate a Structure of the Floorplan with the specified operations          
FloorPlan.BaseX=BaseX; %Coordinate of the overall core
FloorPlan.BaseY=BaseY; %Coordinate of the overall core

%Adjustment needed 
BaseXChange= BaseX - BaseX_current
BaseYChange= BaseY - BaseY_current

FloorPlan.Labels= unit_name_1;
FloorPlan.w=width_1.*sf; %unit widths
FloorPlan.h=height_1.*sf; %unit heights
FloorPlan.x=x_left_1.*sf+BaseXChange; % unit x cordinates
FloorPlan.y=y_bottom_1.*sf+BaseYChange; %unit y cordinates
...
FloorPlanCell={unit_name_1,width_1.*sf,height_1.*sf,...
                x_left_1.*sf+BaseXChange,...
                y_bottom_1.*sf+BaseYChange, ...
